...
# %% auto 0
__all__ = ['read_vcf', 'hmp_table_to_matrix', 'd_to_df', 'df_to_d', 'd_to_t', 't_to_d', 'df_to_t', 't_to_df',
           'convert_connections', 'order_connections', 'intersect_cxn_gff_nodes', 'acgt_filter_taxa',
...
def  hmp_table_to_matrix(
    ...
    def _mk_iupac():
        "Returns a dict mapping IUPAC characters to acgt percentages"
        nuc2np = {
            'A': np.array([1., 0, 0, 0]), 
            'C': np.array([0., 1, 0, 0]), 
            'G': np.array([0., 0, 1, 0]), 
        ...
        iupac = {e:np.concatenate( [ nuc2np[ee][:, None] for ee in iupac[e]], axis=1 
                                  ).mean(axis=1) for e in iupac}
        # merge
        nuc2np |= iupac 
        iupac = nuc2np
        return iupac
